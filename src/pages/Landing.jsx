...
export default function Landing() {
  return (
    <main className="relative min-h-dvh pt-24">
      {/* background accents */}
      <div aria-hidden="true" className="pointer-events-none absolute top-[-12rem] left-0 -ml-px -translate-x-24 h-[34rem] w-[34rem] rounded-full blur-3xl"
           style={{ background: 'radial-gradient(closest-side, rgba(59,130,246,.25), transparent)' }} />
      <div aria-hidden="true" className="pointer-events-none absolute bottom-[-14rem] right-0 h-[38rem] w-[38rem] rounded-full blur-[90px]"
           style={{ background: 'radial-gradient(closest-side, rgba(34,211,238,.22), transparent)' }} />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-8">
        {/* NAV */}
        <header className="sticky top-0 z-40 -mt-24 pt-24 pb-3 backdrop-blur supports-[backdrop-filter]:bg-white/5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <img src={logoFull} alt="Operon" className="h-9 sm:h-10" />
              <span className="text-xs sm:text-sm text-operon-muted hidden sm:block">Modern property operations</span>
            </div>
            <div className="flex items-center gap-2">
              <Link to="/login" className="px-3 py-2 text-sm rounded-md border border-white/15 text-operon-muted hover:text-white">Log in</Link>
              <a href="/book" className="px-4 py-2 text-sm rounded-md bg-operon-blue text-white hover:bg-blue-400">Book demo</a>
            </div>
          </div>
        </header>
        ...
        <div className="grid md:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <motion.section variants={fade} initial="initial" animate="animate">
            ...
            <h1 className="text-4xl sm:text-5xl font-extrabold leading-tight text-operon-charcoal">
              Slash guest wait time. <span className="bg-gradient-to-r from-blue-500 to-cyan-400 bg-clip-text text-transparent">Zero requests lost.</span>
            </h1>
        ...
        <section className="mt-12 mb-20 text-center">
          ...
          <div className="mt-4 flex items-center justify-center gap-3">
            ...
            <Link to="/login" className="px-5 py-2.5 rounded-lg border border-white/15 text-operon-muted hover:text-white">Log in</Link>
          </div>
        </section>
      </div>
